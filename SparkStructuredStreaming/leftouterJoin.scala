...
// streaming dataframe
    ...
    val joinType = "leftouter"  // only works if left table is streaming and if not it will throw out an error

    val finaldf = refineddf.join( memberdf, joinCond, joinType)
