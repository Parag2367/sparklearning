...
object example1 extends App {
    ...
    // this will give a schema which will be under values, for eg : values.order_id, values.order_status, values. , etc we will have to pass values. everytime
    // so to avoid it and we do following

    val refinedf = valuedf.select("value.*")

    // this will give us a root structure just like normal schema

    val windowaggdf = refinedf.
    withWatermark("order_date", "30 minute").  // added watermark to give a limit to the application running time
